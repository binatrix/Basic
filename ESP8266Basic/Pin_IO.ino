...
float UniversalPinIO(String PinCommand, String PinDesignaor, float PinValue)
{
  ...
  if (pin == 0)
  {
    ...
    if (PinDesignaor != F("0"))
    { //  If PinDesignator is "0", pin is already 0 and we can skip the next 11 if statements and toUpper() function.
      ...
      if (PinDesignaor == F("D0"))
        pin = 16;
      if (PinDesignaor == F("D1"))
        pin = 5;
      if (PinDesignaor == F("D2"))
        pin = 4;
      if (PinDesignaor == F("D3"))
        pin = 0;
      if (PinDesignaor == F("D4"))
        pin = 2;
      if (PinDesignaor == F("D5"))
        pin = 14;
      if (PinDesignaor == F("D6"))
        pin = 12;
      if (PinDesignaor == F("D7"))
        pin = 13;
      if (PinDesignaor == F("D8"))
        pin = 15;
      if (PinDesignaor == F("RX"))
        pin = 3;
      if (PinDesignaor == F("TX"))
        pin = 1;
    }
  }
  ...
  if (PinCommand != F("po") & PinCommand != F("pi") & PinCommand != F("pip") & PinCommand != F("pwi") & PinCommand != F("pwo") & PinCommand != F("servo") & PinCommand != "")
  {
  ...
  {
    if (PinCommand == F("pi") | PinCommand == F("pip"))
    {
      if ((PinListOfStatus[pin] == F("po")) | (PinListOfStatus[pin] == F("pi")) | (PinListOfStatus[pin] == F("pip")) | (PinListOfStatus[pin] == F("pwi")) | (PinListOfStatus[pin] == F("pwo")) | (PinListOfStatus[pin] == F("servo")) | (PinListOfStatus[pin] == ""))
      {
        SetThePinMode(PinCommand, pin);
        PinListOfStatus[pin] = PinCommand;
      }
  ...
  else if (PinCommand == F("pi") | PinCommand == F("pip"))
  {
  ...
  else if (PinCommand == F("pwo"))
    analogWrite(pin, PinValue);
  else if (PinCommand == F("servo"))
    servoWrite(pin, PinValue);
  else if (PinCommand == F("ai"))
    return analogRead(A0);
  else if (PinCommand == F("interrupt"))
    return pin;
  return 0;
}
...
{
  if (PinListOfStatus[pin] == PinCommand)
    return;
  if (PinListOfStatus[pin].startsWith("[") & (PinCommand == "pi" | PinCommand == "pip"))
    return;
  ...
  if (PinCommand != F("servo"))
    servos[((pin > 5) ? pin - 6 : pin)].detach(); // Shifts any high range pins down to the servo[6] to servo[10] positions.

  if (PinCommand == F("po") | PinCommand == F("pwo"))
    pinMode(pin, OUTPUT);
  if (PinCommand == F("pi") | PinCommand == F("pwi"))
    pinMode(pin, INPUT);
  if (PinCommand == F("pip"))
