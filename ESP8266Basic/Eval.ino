...
int function_callback(void *user_data, const char *name, const int num_args, const float *args, float *value, String **args_str, String *value_str)
{
  ...
  */
  ...
  else if (fname == F("hex"))
  {
    ...
    { // example of the hex(value, nb_digits)
  ...
  else if (fname == F("mqtt.setup") && num_args >= 1)
  {
    ...
    if (num_args >= 2)
      MQTTport = args[1];
    if (num_args >= 3)
      MQTTUser = String(*args_str[2]);
    if (num_args == 4)
      MQTTPass = String(*args_str[3]);
  ...
  else if (fname == F("wifi.ssid") && num_args == 1)
  {
    ...
    // set return value
    if (numberOfWifiScanResults < args[0])
      return PARSER_STRING;
  ...
  else if (fname == F("wget") && num_args > 0)
  {
    ...
    {
      if (num_args == 1)
        *value_str = FetchWebUrl(*args_str[0], 80);
      else if (num_args == 2)
        *value_str = FetchWebUrl(*args_str[0], args[1]);
      return PARSER_STRING;
    }
    else
    {
  ...
  else if (fname == F("temp") && num_args < 2)
  {
    ...
    if (num_args == 0)
    {
      ...
      for (i = 0; sensors.getAddress(deviceAddr, i); i++) // Loop while devices are found
      {
        delay(0);
        if (i > 0)
          tmp.concat(" "); // If not the first device, add a separator space
        for (xx = 0; xx < 8; xx++)
        {
          if (deviceAddr[xx] < 16)
            tmp.concat("0");
  ...
  else if (fname.startsWith(F("spi."))) // block SPI functions; this reduces the number of compares
  {
    ...
    else if (fname == F("setfrequency") && num_args == 1)
    { // let a = spi.write(value_number) -> send and receive a  single byte
      SPI.setFrequency(args[0]);
      *value = -1;
      return PARSER_TRUE;
    }
    else if (fname == F("setmode") && num_args == 1)
    { // let a = spi.write(value_number) -> send and receive a  single byte
    ...
    else if (fname == F("byte") && num_args == 1)
    { // let a = spi.write(value_number) -> send and receive a  single byte
      *value = SPI.transfer(args[0]);
      return PARSER_TRUE;
    }
    else if (fname == F("string") && num_args == 2)
    { // let a$ = spi.string(value_string, length) -> send and receive a buffer of 'length' bytes
      String ret;
      char c;
      ret.reserve(args[1]);
      for (i = 0; i < args[1]; i++)
    ...
    else if (fname == F("hex") && num_args == 2)
    { // let a$ = spi.hex(value_string, length) -> send and receive a buffer of 'length' bytes; each byte is a 2 char hex string
      if (args_str[0] == NULL)
      {
  ...
  else if (fname.startsWith(F("ir."))) // block Infrared functions; this reduces the number of compares
  {
    ...
    else if (fname == F("send") && num_args == 1)
    {
      ...
      if (IRmfgCodeForOutput == "NEC")
        irsend->sendNEC(HexToLongInt(IRhexCodeForOutput), (int)IRbytesCodeForOutput);
      if (IRmfgCodeForOutput == "SONY")
        irsend->sendSony(HexToLongInt(IRhexCodeForOutput), (int)IRbytesCodeForOutput);
#if defined(BASIC_TFT)
      if (IRmfgCodeForOutput == "LG")
        irsend->sendLG(HexToLongInt(IRhexCodeForOutput), (int)IRbytesCodeForOutput);
      if (IRmfgCodeForOutput == "PANASONIC")
        irsend->sendPanasonic(HexToLongInt(IRhexCodeForOutput), (int)IRbytesCodeForOutput);
      if (IRmfgCodeForOutput == "SAMSUNG")
        irsend->sendSAMSUNG(HexToLongInt(IRhexCodeForOutput), (int)IRbytesCodeForOutput);
#endif
  ...
  else if (fname.startsWith(F("oled."))) // block TFT functions; this reduces the number of compares
  {
    ...
    else if (fname == F("width") && num_args == 1)
    {
      *value = display.getStringWidth(*args_str[0]);
      return PARSER_TRUE;
    }
    else if (fname == F("align") && num_args == 1)
    {
    ...
    else if (fname == F("display") && num_args == 0)
    {
      display.display();
      return PARSER_STRING;
    }
    else if (fname == F("flip") && num_args == 0)
    {
  ...
  else if (fname.startsWith(F("tft."))) // block TFT functions; this reduces the number of compares
  {
    ...
    else if (fname == F("bmp") && num_args > 0)
    {
      ...
      if (args_str[0] == NULL)
      {
        SendErrorMsg(F("tft.bmp() : The first argument must be a string!"));
        return PARSER_FALSE;
      }
      if (num_args > 2)
      {
    ...
    else if (fname == F("obj.button") && num_args >= 5)
    {
      ...
      if (args_str[0] == NULL)
      {
        SendErrorMsg(F("tft.obj.button() : The first argument must be a string!"));
        return PARSER_FALSE;
      }
    ...
    else if (fname == F("obj.label") && num_args >= 5)
    {
      ...
      if (args_str[0] == NULL)
      {
        SendErrorMsg(F("tft.obj.label() : The first argument must be a string!"));
        return PARSER_FALSE;
      }
    ...
    else if (fname == F("obj.checkbox") && num_args >= 4)
    {
      ...
      if (args_str[0] == NULL)
      {
        SendErrorMsg(F("tft.obj.checkbox() : The first argument must be a string!"));
        return PARSER_FALSE;
      }
    ...
    else if (fname == F("obj.radio") && num_args >= 4)
    {
      ...
      if (args_str[0] == NULL)
      {
        SendErrorMsg(F("tft.obj.radio() : The first argument must be a string!"));
        return PARSER_FALSE;
      }
    ...
    else if (fname == F("obj.toggle") && num_args >= 5)
    {
      ...
      if (args_str[0] == NULL)
      {
        SendErrorMsg(F("tft.obj.toggle() : The first argument must be a string!"));
        return PARSER_FALSE;
      }
      if (args_str[1] == NULL)
      {
        SendErrorMsg(F("tft.obj.toggle() : The second argument must be a string!"));
        return PARSER_FALSE;
      }
    ...
    else if (fname == F("obj.setlabel") && num_args == 2)
    {
      ...
      if (args_str[1] == NULL)
      {
        SendErrorMsg(F("tft.obj.setLabel() : The second argument must be a string!"));
        return PARSER_FALSE;
      }
  ...
  else if (fname.startsWith(F("debug."))) // block DEBUG functions; this reduces the number of compares
  {
    ...
    else if (fname == F("object") && num_args > 1)
    {
      if (args_str[0] == NULL)
      {
        SendErrorMsg(F("debug.object() : The first argument must be a string!"));
        return PARSER_FALSE;
      }
  ...
  else if (fname.startsWith(F("sound.")))
  {
    fname = fname.substring(6); // skip the term tft.
    if (fname == F("pin") && num_args == 1)
    {
      sound.setPin(args[0]);
      return PARSER_TRUE;
    }
    else if (fname == F("play") && num_args == 1)
    {
      sound.playMelody(*args_str[0]);
      return PARSER_TRUE;
    }
    else if (fname == F("stop") && num_args == 0)
    {
      sound.stop();
      return PARSER_TRUE;
    }
